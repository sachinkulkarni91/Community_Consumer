...
const EnterEmail = ({email, setEmail, handleForgotPassword} : Props) => {
  return (
   <div className='w-3/4 max-w-[550px] bg-primary rounded-4xl flex flex-col text-lightText'> 
          ...
          <Input label="Email" type="email" id="Email" placeholder="Enter your email" value={email} setValue={setEmail} />


        <div className='flex items-center gap-16'>
           <div className='flex-2/3 text-left text-md'><Link to='/login' className='text-[#009FDA]'>Back to Login</Link></div>
          <button className="flex-1/3 min-w-[180px] text-sm text-white bg-KPMG py-2 px-2 rounded-3xl w-full flex justify-center items-center gap-1 transition-all duration-300 hover:bg-white hover:text-KPMG border-2 border-KPMG my-8" onClick={() => {handleForgotPassword()}}>
          <svg
